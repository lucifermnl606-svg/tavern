...
<head>
    ...
    <title>Tavern RPG - v8.9.3 (Status & Memory Fix)</title>
    <script src="https://www.gstatic.com/firebasejs/9.6.1/firebase-app-compat.js"></script>
    <script src="https://www.gstatic.com/firebasejs/9.6.1/firebase-database-compat.js"></script>
    <style>
        :root { --gold: #f1c40f; --red: #e74c3c; --blue: #3498db; --green: #2ecc71; --purple: #9b59b6; --bg: #050510; --card: #0a0a1a; }
        body { font-family: 'Segoe UI', sans-serif; background: var(--bg); color: white; margin: 0; padding: 15px; text-align: center; transition: 0.3s; }
        .zombie-mode { filter: grayscale(0.5) sepia(0.5) !important; background: #1a2310 !important; color: #a2b78d !important; }
        
        #turn-indicator { position: fixed; top: 15px; right: 15px; background: rgba(0,0,0,0.9); padding: 10px 15px; border-radius: 25px; border: 2px solid #333; z-index: 500; font-weight: bold; font-size: 12px; }
        .status-ready { border-color: var(--gold); color: var(--gold); } 
        .status-active { border-color: var(--green); color: var(--green); box-shadow: 0 0 10px var(--green); } 
        .status-wait { border-color: #444; color: #666; }

        .death-box { background: black; padding: 30px; border: 3px solid var(--gold); border-radius: 20px; width: 80%; max-width: 400px; box-shadow: 0 0 30px #f00; }
        #death-screen { display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(80, 0, 0, 0.95); z-index: 10000; justify-content: center; align-items: center; flex-direction: column; }

        .input-dark { width: 80%; padding: 12px; margin: 8px 0; background: #111; border: 1px solid #333; color: white; border-radius: 8px; }
        .btn-gold { background: var(--gold); color: black; border: none; padding: 15px; border-radius: 8px; font-weight: bold; cursor: pointer; width: 100%; margin-top: 10px; }
        .btn-end-turn { background: var(--red); color: white; margin-top: 20px; border: none; padding: 15px; border-radius: 8px; font-weight: bold; width: 100%; cursor: pointer; display: none; }
        
        .stats-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 5px; margin: 15px 0; }
        .stat-box { background: var(--card); padding: 8px; border-radius: 8px; border: 1px solid #1e3799; font-size: 14px; display: flex; align-items: center; justify-content: center; min-height: 25px; }
        
        .ability-grid { display: grid; grid-template-columns: 1fr; gap: 10px; margin-top: 10px; }
        .ability-btn { background: #1a1a4a; border: 1px solid var(--blue); padding: 15px; border-radius: 10px; color: white; cursor: pointer; text-align: left; position: relative; }
        .ability-btn b { color: var(--gold); font-size: 15px; display: block; margin-bottom: 2px; }
        .ability-btn i { font-size: 12px; color: #bbb; display: block; font-style: normal; }
        .ability-btn .mana-tag { position: absolute; right: 15px; top: 15px; font-weight: bold; color: var(--blue); }
        .ability-btn.disabled { opacity: 0.2; pointer-events: none; filter: grayscale(1); }

        #dm-console { display: none; background: #222; margin-top: 20px; padding: 15px; border-radius: 10px; border: 1px dashed var(--gold); text-align: left; }
        ...
        .modal { background: var(--card); padding: 20px; border-radius: 15px; width: 80%; }
    </style>
</head>
<body id="game-body">
    ...
    <div id="death-screen">
        <div class="death-box">
            <h1 id="death-message" style="color:var(--red); font-size: 24px; margin-bottom: 15px;">U bent Dood 💀</h1>
            <p>Je bent uitgeschakeld.</p>
            <button class="btn-gold" id="res-btn" onclick="resurrect()">KOM TERUG (20 COINS)</button>
            <button class="btn-gold" style="background:#444; color:white;" onclick="permDeath()">NIEUW KARAKTER</button>
        </div>
    </div>

    <div id="login-screen">
        <h1 onclick="document.getElementById('dm-console').style.display='block'" style="color:var(--gold); cursor:pointer;">THE TAVERN 9.4</h1>
        <input type="text" id="login-name" placeholder="Naam..." class="input-dark">
        <input type="password" id="login-pass" placeholder="Wachtwoord" class="input-dark">
        <select id="login-class" class="input-dark">
            <option value="Wizard">Wizard</option><option value="Bard">Bard</option><option value="Rogue">Rogue</option>
            <option value="Paladin">Paladin</option><option value="Wench">Wench</option><option value="Bouncer">Bouncer</option>
            <option value="Monk">Monk</option><option value="Necro">Necro</option><option value="Druid">Druid</option>
            <option value="Gravedigger">Gravedigger</option>
        </select>
        <button class="btn-gold" onclick="enterTavern()">START</button>
    </div>

    <div id="main-app" style="display:none;">
        <h2 id="disp-name" style="color:var(--gold); margin:0;"></h2>
        <div id="zombie-timer-display" style="color:var(--green); font-weight:bold; margin-bottom:10px;"></div>
        <div class="stats-grid" id="stat-container"></div>
        <div class="ability-grid" id="ability-container"></div>
        <button id="end-turn-btn" class="btn-end-turn" onclick="passTurn()">🛑 EINDIG MIJN BEURT</button>
        
        <div id="dm-console">
            <h4 style="margin:0; color:var(--gold)">DM PANEL</h4>
            <div id="player-status-list" style="font-size: 12px; color: #ccc; margin: 10px 0;"></div>
            <button class="btn-gold" style="background:#444; color:white; padding:5px;" onclick="db.ref('accounts').once('value', s => s.forEach(p => db.ref('accounts/'+p.key).update({isLocked:false, pendingFX:null, sleepTimer:0, zombieTimer:0})))">UNBLOCK & CLEANSE ALL</button>
        </div>
    </div>
    ...
    <div id="target-overlay" class="overlay">
        <div class="modal">
            ...
            <button class="btn-gold" onclick="closeOverlay()" style="background:#666">ANNULEREN</button>
        </div>
    </div>
...
<script>
    ...
    function execute(targetName) {
        ...
        if (a.fx === 'zombify' && myData.class === "Zombie") {
            db.ref('accounts/' + targetName).once('value', snap => {
                const targetData = snap.val();
                if (!targetData.pendingFX || targetData.pendingFX !== 'poison') { 
        ...
        if(a.fx && a.fx !== 'lock' && a.fx !== 'zombify') {
            ...
            if(a.fx === 'sleep') updates.sleepTimer = 2; // Hier wordt de timer gezet op 2
        }
        ...
        db.ref('accounts').once('value', snap => {
            ...
            if(npData) {
                ...
                if(npData.pendingFX === "poison") {
                    db.ref('accounts/' + nextPlayer + '/hp').transaction(h => h - 3);
                }

                // Verbeterde Slaap Logica
                if(npData.pendingFX === "sleep"){
                    let st = (npData.sleepTimer || 0) - 1;
                    if(st <= 0) {
                        turnUpdates.sleepTimer = 0;
                        turnUpdates.pendingFX = null; // Maak effect leeg zodat de UI ontgrendelt
                    } else {
                ...
                if(npData.class === "Zombie") {
                    ...
                    if(zt <= 0) {
                        ...
                        alert(nextPlayer.toUpperCase() + " is geen Zombie meer! (Terug naar " + (oldHP-30) + " HP)");
                    } else {
                ...
                if(Object.keys(turnUpdates).length > 0) {
                    db.ref('accounts/' + nextPlayer).update(turnUpdates);
                }
            }
